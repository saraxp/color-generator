...
interface ClipBoardButtonProps {
  ChangeClipBoardStatus: (value: boolean) => void;
  ClipBoardStatus: boolean;
}

const ClipBoardButton = ({ClipBoardStatus, ChangeClipBoardStatus}: ClipBoardButtonProps) => {
  function handleClick() {
    if(ClipBoardStatus === false){
      ChangeClipBoardStatus(true);
    } else {
      ChangeClipBoardStatus(false);
    }
  }
  return (
    <>
      <button 
        onClick={handleClick}
        className='bg-[FFF9F9] border-2 rounded-full border-[#936666] w-[37px] h-[35px] flex justify-center items-center'
      >
